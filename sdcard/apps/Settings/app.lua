...
local function rowRect(i)
  return 10, 10 + (i - 1) * (ROWH + 6), 300, ROWH
end

local function title(t)
  gfx.fillRect(0, 0, SCREEN_W, 26, COLOR.NAVY)
  gfx.text(8, 8, t, COLOR.WHITE, 1)
end
...
local function drawMain()
  ...
  for i, name in ipairs(rows) do
    ...
    gfx.text(x + 10, y + 15, name, COLOR.BLACK, 2)
    if val ~= "" then gfx.text(310 - gfx.textW(val, 1) - 12, y + 18, val, COLOR.BLACK, 1) end
    gfx.text(x + 284, y + 16, ">", COLOR.DGRAY, 1)
  end
end
...
local function drawLed()
  ...
  gfx.fillRect(14, 172, 292, 48, COLOR.FACE)
  gfx.rect(14, 172, 292, 48, COLOR.DGRAY)
  gfx.text(160 - math.floor(gfx.textW("LED OFF", 2) / 2), 186, "LED OFF", COLOR.BLACK, 2)
  gfx.text(8, 228, "< BACK", COLOR.WHITE, 2)
end
...
local function drawBright()
  ...
  gfx.fillRect(30, 80, 260, 40, COLOR.BLACK)
  gfx.fillRect(32, 82, math.max(2, math.floor(256 * b / 255)), 36, COLOR.YELLOW)
  gfx.rect(30, 80, 260, 40, COLOR.DGRAY)
  gfx.text(160 - math.floor(gfx.textW(tostring(b), 3) / 2), 130, tostring(b), COLOR.WHITE, 3)
  gfx.fillRect(40, 170, 100, 50, COLOR.FACE)
  gfx.rect(40, 170, 100, 50, COLOR.DGRAY)
  gfx.text(90 - 24, 184, "- 25", COLOR.BLACK, 2)
  gfx.fillRect(180, 170, 100, 50, COLOR.FACE)
  gfx.rect(180, 170, 100, 50, COLOR.DGRAY)
  gfx.text(230 - 24, 184, "+ 25", COLOR.BLACK, 2)
  gfx.text(8, 228, "< BACK", COLOR.WHITE, 2)
end
...
local function drawWifi()
  gfx.cls(COLOR.FACE)
  title("Settings > WiFi")
  gfx.text(8, 32, wifi.status() .. "  " .. wifi.ip(), COLOR.BLACK, 1)
  ...
  for i, n in ipairs(nets) do
    ...
    local y = 96 + (i - 1) * 34
    gfx.fillRect(8, y, 304, 32, COLOR.WHITE)
    gfx.rect(8, y, 304, 32, COLOR.DGRAY)
    local lock = ""
    if n.lock then lock = "[x]" end
    gfx.text(14, y + 11, n.ssid, COLOR.BLACK, 1)
    gfx.text(250, y + 11, lock .. " " .. n.rssi, COLOR.DGRAY, 1)
  end
...
function onTouch(t, x, y)
  ...
  elseif state == "led" then
    if y < 40 or (y > 220 and x < 120) then state = "main"; drawMain(); return end
    for i, p in ipairs(presets) do
      local col = (i - 1) % 4
      local row = math.floor((i - 1) / 4)
      local px = 14 + col * 76
    ...
    if x >= 14 and x <= 303 and y >= 175 and y <= 217 then
      sys.ledOff()
      beep(500, 40)
    end
  elseif state == "bright" then
    if y < 40 or (y > 220 and x < 120) then state = "main"; drawMain(); return end
    if x >= 40 and x <= 137 and y >= 173 and y <= 217 then
      sys.setBacklight(math.max(10, sys.getBacklight() - 25))
    elseif x >= 180 and x <= 277 and y >= 173 and y <= 217 then
